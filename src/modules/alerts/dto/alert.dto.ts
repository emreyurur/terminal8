...
export class CreateAlertDto {
  ...
  @Matches(/^[0-9a-fA-F]{64}$/, {
    message: "poolId must be a 64 character hex pool id",
  })
  poolId: string;
  ...
  @ApiProperty({
    description:
      "Value that triggers the alert (percent for IMPERMANENT_LOSS_PCT)",
  })
  ...
  @ApiPropertyOptional({
    enum: ["A", "B"],
    description: "Pool asset the price / value is expressed in (default A)",
  })
  @IsIn(["A", "B"])
  @IsOptional()
  quoteSide?: QuoteSide;
...
export class UpdateAlertDto {
  ...
  @ApiPropertyOptional({
    enum: ["ACTIVE", "PAUSED"],
    description: "ACTIVE re-arms a triggered alert",
  })
  @IsIn(["ACTIVE", "PAUSED"])
  @IsOptional()
